...
def run_mc(scope_groups=None,N_runs=500,refresh=True,threshold_conf = 0.8,conf_int = 0.9):
    clm.load()
    fun_handle = design.update
    output_fields = {WATCH_GROUP:WATCH_FIELDS}
    input_table = SQL_TABLE ;  output_table = SQL_OUTPUT
    ctbl = monte_carlo(fun_handle=fun_handle,output_fields=output_fields,input_table=input_table,
                output_table=output_table,conf_int=conf_int,con=clm.engine,
                threshold_conf=threshold_conf,N_runs=N_runs,refresh=refresh,
